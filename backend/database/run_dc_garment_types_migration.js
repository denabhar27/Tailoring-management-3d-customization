...
INSERT IGNORE INTO dry_cleaning_garment_types (garment_name, garment_price, description, is_active) VALUES
...
async function runMigration() {
  console.log('Starting dry_cleaning_garment_types migration...');
  ...
  try {
    ...
    const countResult = await new Promise((resolve, reject) => {
      db.query('SELECT COUNT(*) as count FROM dry_cleaning_garment_types', (err, result) => {
        if (err) reject(err);
        else resolve(result);
      });
    });
    ...
    if (countResult[0].count === 0) {
      ...
      await new Promise((resolve, reject) => {
        db.query(insertDefaultDataSQL, (err, result) => {
          if (err) reject(err);
          else resolve(result);
        });
      });
      ...
      console.log('Table already has data, skipping default inserts');
    }
